...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create clone-twitter-2078
  fly apps create clone-twitter-2078-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app clone-twitter-2078
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app clone-twitter-2078-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'clone-twitter-2078-staging' does not match app name in config file 'clone-twitter-2078'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name clone-twitter-2078-db
  fly postgres attach --app clone-twitter-2078 clone-twitter-2078-db

  fly postgres create --name clone-twitter-2078-staging-db
  fly postgres attach --app clone-twitter-2078-staging clone-twitter-2078-staging-db
  ```
